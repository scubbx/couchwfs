function(head, req) {
  ...
  var m_url = "http://gisforge.no-ip.org:5984/wfs2/_design/wfs/_spatial/_list/wfspoint/point?";
  var m_ProviderName = "Markus Mayr";
  var m_CoordSystem = "EPSG:4326";
  var m_FeatureTypeName = "All Points";
  var m_FeatureTypeTitle = "All Points";
  ...
  if (req.query.REQUEST == "GetCapabilities" || req.query.request == "GetCapabilities"){
    ...
    send('<WFS_Capabilities xmlns="http://www.opengis.net/wfs" xmlns:ogc="http://www.opengis.net/ogc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.1.0" updateSequence="0" xsi:schemaLocation="http://www.opengis.net/wfs http://schemas.opengis.net/wfs/1.1.0/WFS-capabilities.xsd">');
      ...
      send('<Capability>');
        send('<Request>');
          ...
          send('<GetFeature>');
            send('<ResultFormat>');
              send('<GML3/>'); // ACHTUNG: welcher Standard wird nun tatsächlich verwendet? SimpleFeatures?
            send('</ResultFormat>');
            send('<DCPType>');
              send('<HTTP>');
                send('<Get onlineResource="'+m_url+'"/>'); // abermals default URLs des WFS
    ...
    send('<schema xmlns:cwfs="'+m_cwfsNS+'" xmlns:ogc="http://www.opengis.net/ogc" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/2001/XMLSchema" xmlns:gml="http://www.opengis.net/gml" targetNamespace="http://mapserver.gis.umn.edu/mapserver" elementFormDefault="qualified" version="0.1">');  // !! "xmlns:ms" geaendert in "xmlns:cwfs"
      ...
      // START ELEMENT ------------
      send('<element name="polygon" type="cwfs:lineType" substitutionGroup="gml:_Feature"/>');
      send('<complexType name="lineType">');
        send('<complexContent>');
  ...
  // preprocess response from the database
    ...
    while ( row = getRow() ) {
      var coorddata = row.geometry.coordinates;
      ...
      gmlCoords = coorddata[0]+","+coorddata[1];
    ...
    send('<wfs:FeatureCollection xmlns:wfs="http://www.opengis.net/wfs" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.example.com/myns http://localhost:8888/wfsservlet?featureTypeId=1 http://www.opengis.net/wfs ../wfs/1.1.0/WFS-basic.xsd" >');
      ...
      for(var j=0;j<elementlist.length;j++){
        ...
        send('<gml:featureMember xmlns:gml="http://www.opengis.net/gml" >');
          send('<cwfs:LINES xmlns:cwfs="'+m_cwfsNS+'" fid="'+j+'" >');
          ...
          // END ATTRIBUTES
            ...
            send('<cwfs:SHAPE>');
              send('<gml:Point xmlns:gml="http://www.opengis.net/gml" srsName="'+m_CoordSystem+'" >');
